...
export async function GET(req: Request) {
  ...
  if (confirm !== 'true') {
    return NextResponse.json({
      message: "Please add '?confirm=true' to the URL to initialize database seeding. WARNING: This will clear existing tables."
    }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Database seeded successfully with all 5 role accounts, mock orders, follow-ups, and payments."
    });
    ...
    return NextResponse.json({
      success: false,
      error: err.message || "Failed during database seeding operations."
    }, { status: 500 });
  }
}
